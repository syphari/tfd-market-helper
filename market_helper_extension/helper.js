...
(function() {
  ...
  // When loaded as part of a Chrome extension, chrome.storage is available.
  if (typeof chrome !== 'undefined' && chrome.storage) {
    chrome.storage.local.get('marketData', (result) => {
      modules = result.marketData || [];
      initializeUI(modules);
    });
  } else {
    // Not running inside extension (e.g. opened directly). Show a message.
    const container = document.getElementById('cardsContainer');
    if (container) {
      container.innerHTML = '<p style="padding:20px; text-align:center;">No data available. Please open this page via the extension.</p>';
    }
  }
  ...
  function initializeUI(data) {
    ...
    function updateResults() {
      ...
      const filtered = modules.filter(mod => {
        ...
        // Attribute filtering (for ancestor modules)
        const attrMatch = selectedAttrs.every(attr => (mod.attributes || []).includes(attr));
        // Socket filter
    ...
    if (clearModuleNamesBtn) {
      ...
      filtered.forEach(mod => {
        ...
        (mod.stats || []).forEach(stat => {
          const parts = stat.raw.trim().split(/\s+/);
